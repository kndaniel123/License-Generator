...
class Key:
    ...
    def get_license_time_from_amount(self):
        ...
        def enter_amount():
            ...
            raw_input_amount = 500 #raw_input()
            try:
                ...
                if input_amount:
                    ...
                    with open("license_rates.json", "r") as file:
                        ...
                        if input_amount in license_rates[
                                'periods_n_amount'].values():
                            # get the license time period
                            for licence_period, license_amount in license_rates[
                                    'periods_n_amount'].items():
                                if input_amount == license_amount:
                                    # if license time in seconds
                                    license_time = license_rates[
                                        'period_n_time_in_seconds'][
                                            licence_period]
                                    return {
                                        'period_name': licence_period,
                                        'time_in_seconds': license_time
                                    }
                        else:
                            print(
                                "Entered amount does not match any license period n\
								The rates are as follows:")
                            for license_period in license_rates[
                                    'periods_n_amount'].keys():
    ...
    def add_generated_key(self, license_key_dict):
        if self.check_availability():
            ...
            with open("generated_keys.json", "r+") as file:
                data = json.load(file)
                data["generated_keys"][
                    license_key_dict['unique_license_key']] = license_key_dict
                file.seek(0)  # rewind
                json.dump(data, file)
                file.truncate()
    ...
    def main(self):
        ...
        print(
            "Sucessfully generated and saved a {} licence.Please commit the changes and push the changes online\
